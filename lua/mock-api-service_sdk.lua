...
require("mock-api-service_types")
...
local MockApiServiceSDK = {}
MockApiServiceSDK.__index = MockApiServiceSDK
...
MockApiServiceSDK._make_feature = _make_feature


function MockApiServiceSDK.new(options)
  local self = setmetatable({}, MockApiServiceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MockApiServiceSDK:options_map()
...
function MockApiServiceSDK:get_utility()
  return Utility.copy(self._utility)
end


function MockApiServiceSDK:get_root_ctx()
  return self._rootctx
end


function MockApiServiceSDK:prepare(fetchargs)
...
function MockApiServiceSDK:direct(fetchargs)
...
-- by passing it.
function MockApiServiceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MockApiServiceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MockApiServiceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MockApiServiceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MockApiServiceSDK.new(sdkopts)
  sdk.mode = "test"
