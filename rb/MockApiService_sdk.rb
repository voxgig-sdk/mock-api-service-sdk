...
require_relative 'MockApiService_types'
...
class MockApiServiceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MockApiServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MockApiServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MockApiServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MockApiServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MockApiServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MockApiServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MockApiServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MockApiServiceError.new(
        "#{op}_allow",
        "MockApiServiceSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MockApiServiceError => err
    ...
    if fetched.is_a?(Hash)
      status = MockApiServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MockApiServiceError.new(
        "graphql_error", "MockApiServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MockApiServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
