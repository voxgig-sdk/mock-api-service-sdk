# MockApiService SDK Post entity
...
class PostEntity
  ...
  def data_set(args)
    if args
      @_data = MockApiServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MockApiServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Post, Hash] the loaded Post; raises MockApiServiceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MockApiServiceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Post>, Array] the matching Post items; raises MockApiServiceError on failure
