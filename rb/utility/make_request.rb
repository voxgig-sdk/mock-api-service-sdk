...
module MockApiServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MockApiServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = MockApiServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MockApiServiceResponse.new(fetched)
    else
