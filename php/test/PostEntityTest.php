...
class PostEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = MockApiServiceSDK::test($seed, null);
        $seen = iterator_to_array($base->Post(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = MockApiServiceConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = MockApiServiceSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MOCK_API_SERVICE_TEST_POST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function post_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MOCK_API_SERVICE_TEST_POST_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MOCK_API_SERVICE_TEST_POST_ENTID"]);
