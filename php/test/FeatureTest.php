...
// MockApiService SDK feature test
//
...
class FtCtrl extends MockApiServiceControl
{
...
class FtClient
{
    ...
    private MockApiServiceUtility $utility;

    public function __construct(MockApiServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): MockApiServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MockApiServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MockApiServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MockApiServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MockApiServiceConfig::make_feature($name);
    ...
    private static function buildUrl(MockApiServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MockApiServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MockApiServiceResponse($fetched);
        }
    ...
    private function fail(MockApiServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MockApiServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
