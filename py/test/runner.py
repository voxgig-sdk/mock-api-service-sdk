# MockApiService SDK test runner
...
class MockApiServiceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MockApiServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MockApiServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MockApiServiceTestRunner.getenv("MOCK_API_SERVICE_TEST_LIVE")
        override = MockApiServiceTestRunner.getenv("MOCK_API_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MockApiServiceTestRunner.getenv(key)
        ...
        explain = MockApiServiceTestRunner.getenv("MOCK_API_SERVICE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MOCK_API_SERVICE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MockApiServiceTestRunner._test_control is not None:
            return MockApiServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MockApiServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MockApiServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MockApiServiceTestRunner.load_test_control()
...
def load_env_local():
    MockApiServiceTestRunner.load_env_local()


def env_override(m):
    return MockApiServiceTestRunner.env_override(m)


def entity_data(v):
    return MockApiServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MockApiServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MockApiServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MockApiServiceTestRunner.load_test_control()


def live_client_options():
    return MockApiServiceTestRunner.live_client_options()


def live_delay_ms():
    return MockApiServiceTestRunner.live_delay_ms()
