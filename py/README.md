# MockApiService Python SDK

The Python SDK for the MockApiService API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Health(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.MockApiService(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── mockapiservice_sdk.py         -- Main SDK module
