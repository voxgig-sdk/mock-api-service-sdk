...
//     (\`new MockApiServiceSDK(...)\` / \`MockApiServiceSDK.test(...)\` become
//     \`MockApiServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MockApiServiceSDK'
...
const TEST_SEED = {"entity":{"health":{"test01":{"id":"test01"}},"post":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MockApiServiceSDK(...)\` and any
// \`MockApiServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MockApiServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MockApiServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
