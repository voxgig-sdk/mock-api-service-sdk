...
[MCP](https://modelcontextprotocol.io) server exposing the MockApiService SDK as
two agent tools — `mock-api-service_list` and `mock-api-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mock-api-service-mcp)
make build

# 2. Provide credentials via the environment
export MOCK_API_SERVICE_APIKEY=sk_live_xxx
...
```jsonc
// mock-api-service_list: first page of records
{ "entity": "post" }
{ "entity": "post", "query": { } }

// mock-api-service_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mock-api-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mock-api-service \
     -- "$PWD"/dist/*/mock-api-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mock-api-service_list` and `mock-api-service_load` tools now appear
   in new sessions. Ask the agent to *"list post using mock-api-service"*
   and it calls `mock-api-service_list` with `{"entity":"post"}`.
...
```sh
export MOCK_API_SERVICE_APIKEY=sk_live_xxx            # API key
export MOCK_API_SERVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `mock-api-service_list` tool
...
### Call the `mock-api-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mock-api-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mock-api-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MOCK_API_SERVICE_APIKEY` | API key sent with every request. |
| `MOCK_API_SERVICE_BASE` | Optional override of the API base URL. |
