...
// MockApiService SDK retry feature
...
class MockApiServiceRetryFeature extends MockApiServiceBaseFeature
{
    ...
    public function init(MockApiServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MockApiServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MockApiServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MockApiServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
