...
class MockApiServiceMakeError
{
    public static function call(?MockApiServiceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            ...
            $ctx = new MockApiServiceContext([], null);
        }
        $op = $ctx->op ?? new MockApiServiceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MockApiServiceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MockApiServiceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MockApiServiceError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MockApiServiceError) {
