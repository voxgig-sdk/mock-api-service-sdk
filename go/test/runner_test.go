...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MOCK_API_SERVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("MOCK_API_SERVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MOCK_API_SERVICE_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MockApiServiceSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MockApiServiceError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MockApiServiceSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MockApiServiceError{Msg: msg, Code: code}
}
