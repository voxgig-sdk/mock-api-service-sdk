...
import (
	...
	omni "github.com/voxgig-sdk/mock-api-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MockApiServiceError); is {
				out := map[string]any{
					"name":    "MockApiServiceError",
...
func omniCtx(arg any, client *sdk.MockApiServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MockApiServiceSDK); is {
			client = live
