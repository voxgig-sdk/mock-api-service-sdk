// Command MockApiService-cli is a CLI + REPL for the MockApiService SDK,
...
const prompt = "mock-api-service"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "health post user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MOCK_API_SERVICE_APIKEY (api key), MOCK_API_SERVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MOCK_API_SERVICE_APIKEY carries the API key and
	// MOCK_API_SERVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MOCK_API_SERVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MOCK_API_SERVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMockApiServiceSDK(opts)
