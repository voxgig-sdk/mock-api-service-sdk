# mock-api-service-cli

boru-driven command-line client **and** interactive REPL for the MockApiService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mock-api-service-cli)
make build

# 2. See usage (words, entities, env vars)
./mock-api-service-cli --help

# 3. Provide credentials once, via the environment
export MOCK_API_SERVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./mock-api-service-cli load 1 health            # {id:1} shorthand
./mock-api-service-cli load '{id:1}' health       # explicit match map

# 5. Override the API base URL for a single call
MOCK_API_SERVICE_BASE=https://api.example.com ./mock-api-service-cli load 1 health

# 6. No arguments -> interactive REPL
./mock-api-service-cli
mock-api-service> load 1 health
mock-api-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mock-api-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mock-api-service-cli load 1 health
   ```
...
```sh
./mock-api-service-cli load 1 health          # scalar shorthand for {id:1}
./mock-api-service-cli load '{id:1}' health     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `mock-api-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./mock-api-service-cli
mock-api-service> load 1 health
mock-api-service> /help
mock-api-service> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mock-api-service-cli`. |
...
(`./mock-api-service-cli <expr>`) and interactively (the REPL), and expressions compose the
